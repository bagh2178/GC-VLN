...
def _set_config(config):
    ...
    if world_size > 1:
        ...
        device = config.TORCH_GPU_IDS[local_rank]
        config.defrost()
        config.TORCH_GPU_ID = config.TORCH_GPU_IDS[local_rank]
        config.freeze()
        torch.cuda.set_device(device)
...
def init_env(config):
    _set_config(config)
    config.defrost()
    config.TASK_CONFIG.SEED = config.TASK_CONFIG.SEED + config.local_rank
    config.freeze()
