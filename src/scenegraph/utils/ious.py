...
def compute_3d_iou_accurate_batch(bbox1, bbox2):
    ...
    returns: (M, N)
    '''
    # Must expand the box beforehand, otherwise it may results overestimated results
...
def compute_3d_giou_accurate(obj1, obj2):
    ...
    try:
        vols, ious = ops.box3d_overlap(
            torch.from_numpy(box_points1.copy()).unsqueeze(0), 
            torch.from_numpy(box_points2.copy()).unsqueeze(0)
        )
