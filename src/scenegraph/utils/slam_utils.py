...
def to_scalar(d: typing.Union[np.ndarray, torch.Tensor, float]) -> typing.Union[int, float]:
    '''
...
def to_tensor(numpy_array, device=None):
    if isinstance(numpy_array, torch.Tensor):
        return numpy_array
    if device is None:
        return torch.from_numpy(numpy_array.copy())
    else:
        return torch.from_numpy(numpy_array.copy()).to(device)