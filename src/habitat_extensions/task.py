...
class ExtendedInstructionData:
    ...
    # add the instruction DAG parameter
    instruction_DAG: Optional[any] = attr.ib(default=None)
    instruction_DAG_raw: Optional[any] = attr.ib(default=None)
    instruction_DAG_raw_common: Optional[any] = attr.ib(default=None)
    instruction_text_English: Optional[any] = attr.ib(default=None)
    LLM_model: Optional[any] = attr.ib(default=None)
...
class VLNCEDatasetV1(Dataset):
    ...
    def from_json(
        ...
        # multiprocess
        if self.config.SPLIT_NUM >= 0 and self.config.SPLIT_INDEX >= 0:
            episode_num_total = len(deserialized["episodes"])
            episodes_per_split = episode_num_total // self.config.SPLIT_NUM
            remainder = episode_num_total % self.config.SPLIT_NUM

            start_episode = self.config.SPLIT_INDEX * episodes_per_split + min(self.config.SPLIT_INDEX, remainder)
            end_episode = start_episode + episodes_per_split + (1 if self.config.SPLIT_INDEX < remainder else 0)

            deserialized["episodes"] = deserialized["episodes"][start_episode:end_episode]
...
class RxRVLNCEDatasetV1(Dataset):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        if config.EPISODES_ALLOWED is not None:
            ep_ids_before = {ep.episode_id for ep in self.episodes}
            if isinstance(config.EPISODES_ALLOWED, list):
                ep_ids_to_purge = ep_ids_before - set(config.EPISODES_ALLOWED)
            elif isinstance(config.EPISODES_ALLOWED, int):
                episodes_allowed = [str(e) for e in range(-config.EPISODES_ALLOWED)]
