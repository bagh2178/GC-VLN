...
# -----------------------------------------------------------------------------
_C = CN()
_C.ENV_NAME = "GCVLNEnv"
_C.SIMULATOR_GPU_IDS = [0]
_C.TORCH_GPU_IDS = [0]
_C.GPU_NUMBERS = 1
_C.NUM_ENVIRONMENTS = 8
_C.TASK_CONFIG = CN()
_C.TASK_CONFIG.SEED = 100
# -----------------------------------------------------------------------------
# ENVIRONMENT
# -----------------------------------------------------------------------------
_C.ENVIRONMENT = CN()
...
_C.SIMULATOR.SEED = _C.TASK_CONFIG.SEED
_C.SIMULATOR.TURN_ANGLE = 30  # angle to rotate left or right in degrees
_C.SIMULATOR.TILT_ANGLE = 30  # angle to tilt the camera up or down in degrees
...
_C.EVAL.EPISODE_COUNT = -1 # -1 means all

_C.TASK_CONFIG.SIMULATOR = _C.SIMULATOR
_C.TASK_CONFIG.TASK = _C.TASK
_C.TASK_CONFIG.ENVIRONMENT = _C.ENVIRONMENT
_C.TASK_CONFIG.DATASET = _C.DATASET
_C.TASK_CONFIG.PYROBOT = _C.PYROBOT

def _set_config(config):
    '''supple some settings
    '''
    # set the panorama cameras, here we use 12 cameras to generate panorama
    rad = np.pi / 6
    camera_orientations_deg = [[0, 30*i, 0] for i in range(1,12)]
    camera_orientations_rad = [[0, -rad*i, 0] for i in range(1,12)]
    for sensor_type in ["RGB", "DEPTH"]:
        sensor = getattr(config.SIMULATOR, f"{sensor_type}_SENSOR")
        for i in range(len(camera_orientations_deg)):
            camera_template = f"{sensor_type}_{camera_orientations_deg[i][1]}"
            camera_config = deepcopy(sensor)
            camera_config.ORIENTATION = camera_orientations_rad[i]
...
def get_config(
    ...
    :param policy_config: the parameter of the agent policy
    ...
    if config_paths:
        ...
        for config_path in config_paths:
            config.TASK_CONFIG.merge_from_file(config_path)

    if opts:
        config.merge_from_list(opts)

    _set_config(config.TASK_CONFIG)
    if policy_config:
        config.POLICY_CONFIG = policy_config
        config.TORCH_GPU_ID = 0
        config.RESULTS_DIR = 'data/logs/eval_results/'
        # add panorama camera sensor
